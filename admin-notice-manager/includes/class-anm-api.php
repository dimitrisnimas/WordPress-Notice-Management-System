...
class ANM_API {
	...
	public function get_notices( $request ) {
		...
		foreach ( $notices as $notice ) {
			...
			$is_dismissed = false;
			if ( $user_id && $notice->is_dismissible ) {
	...
	public function dismiss_notice( $request ) {
		...
		$notice = ANM_Database::get_notice_for_client( $notice_id, $client->id );
		if ( ! $notice ) {
			return new WP_Error( 'notice_not_found', __( 'Notice not found.', 'admin-notice-manager' ), array( 'status' => 404 ) );
		}

		if ( ! $notice->is_dismissible ) {
			return new WP_Error( 'notice_not_dismissible', __( 'This notice cannot be dismissed.', 'admin-notice-manager' ), array( 'status' => 400 ) );
		}
